/**
 * Scraper: Gas stations in the Campinas region, SP
 *
 * Uses the OpenStreetMap Overpass API to fetch every amenity=fuel
 * inside the Campinas metro-area bounding box and upserts them
 * into the Rotaposto database.
 *
 * Usage:  node scripts/scrape-campinas-stations.mjs
 */
...
// Bounding box covering Campinas and the main surrounding municipalities
// (Sumaré, Americana, Santa Bárbara d'Oeste, Hortolândia, Valinhos, Vinhedo)
// south, west, north, east
const CAMPINAS_BBOX = '-23.13,-47.26,-22.74,-46.95'

const OVERPASS_QUERY = `
[out:json][timeout:60];
(
  node["amenity"="fuel"](${CAMPINAS_BBOX});
  way["amenity"="fuel"](${CAMPINAS_BBOX});
);
out center tags;
`
...
async function main() {
  ...
  try {
    ...
    const count = await client.query(
      `SELECT COUNT(*)::int AS total FROM "Station" WHERE address ILIKE '%Campinas%'`
    )
    console.log(`     Total Campinas stations in DB: ${count.rows[0].total}`)
  } finally {
    await client.end()
  }
}
...
function normalizeElement(el) {
  ...
  // Derive city from OSM tags; fall back to "Campinas"
  const city = tags['addr:city'] || 'Campinas'
  ...
  const id = `campinas-${crypto.createHash('md5').update(osmId).digest('hex').slice(0, 16)}`
