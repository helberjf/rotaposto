/**
 * Scraper: Gas stations in Rio de Janeiro, RJ
 * Usage:  node scripts/scrape-rj-stations.mjs
 */
...
// Bounding box for Rio de Janeiro city
// south, west, north, east
const BBOX = '-23.09,-43.79,-22.74,-43.10'
...
async function main() {
  ...
  try {
    ...
    const count = await client.query(
      `SELECT COUNT(*)::int AS total FROM "Station" WHERE address ILIKE '%Rio de Janeiro%'`
    )
    console.log(`     Total RJ stations in DB: ${count.rows[0].total}`)
  } finally {
    await client.end()
  }
}
...
function normalizeElement(el) {
  ...
  const id = `rj-${crypto.createHash('md5').update(osmId).digest('hex').slice(0, 16)}`
